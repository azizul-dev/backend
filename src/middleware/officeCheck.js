    ...
    for (const office of officeIps) {
      try {
        if (office.includes('/')) {
          ...
          if (officeAddr.kind() === 'ipv6' && officeAddr.isIPv4MappedAddress()) {
            officeAddr = officeAddr.toIPv4Address();
          }
          if (addr.kind() === officeAddr.kind() && addr.toNormalizedString() === officeAddr.toNormalizedString()) {
            return true;
          }
        }
      } catch (e) {}
...
const officeCheck = asyncHandler(async (req, res, next) => {
  ...
  if (setting.geo && setting.geo.enabled && req.body.lat && req.body.lng) {
    const dist = getDistance(req.body.lat, req.body.lng, setting.geo.lat, setting.geo.lng);
